...
import { requestAccounts, interactWithHealthContract, interactWithLifeContract, interactWithTravelContract, interactWithVehicleContract, interactWithGlobalClaimTracker, storeClaimInDatabasefromHealth, ClaimType } from './healthcontractService';
...
const ClaimForm = () => {
  ...
  const handleHealthSubmit = async (e) => {
    ...
    try {
      ...
      } else if (policyDetails.companyName === 'LIC') {
          apiEndpoint = 'http://127.0.0.1:5002/api/v0/add'; // Example endpoint for lic
      }
      ...
      const documentFile = e.target.supportingDocuments.files[0];
      if (documentFile) {
        const documentFormData = new FormData();
        documentFormData.append('file', documentFile);

        const documentResponse = await axios.post(apiEndpoint, documentFormData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        medicalDetails.supportingDocuments = documentResponse.data.Hash;  // Add the file IPFS hash to claim data
        console.log('Supporting document uploaded to IPFS, CID:', documentResponse.data.Hash);
      } else {
        medicalDetails.supportingDocuments = "NA";
      }

      const claimStatus = await interactWithGlobalClaimTracker(ClaimType.Health, policyDetails.policyNumber);
      ...
      if (claimStatus) {
          const tx = await interactWithHealthContract(policyDetails, medicalDetails, bankDetails);
          const receipt = await tx.wait();  // Waits for the transaction to be mined
          await storeClaimInDatabasefromHealth(policyDetails.policyHolderName, policyDetails.policyNumber, policyDetails.companyName, ipfsCid,medicalDetails.supportingDocuments);
          console.log(typeof ClaimType.Health);  
      ...
      console.error("Error filing claim:", error.message || error);
  ...
  const handleLifeSubmit = async (e) => {
    ...
    const medicalDetails = {
      ...
      firAttachment: e.target.fir?.files[0] || "", 
      doctorDetails: e.target.doctorName?.value || "",
      hospitalName: e.target.hospitalName?.value || "",
      hospitalAddress: e.target.hospitalAddress?.value || "",
      hasOtherPolicy: e.target.otherPolicy?.value.toLowerCase() === "yes" ? "yes" : "no",
    ...
    try {
      ...
      const claimStatus = await interactWithGlobalClaimTracker(ClaimType.Life, policyDetails.policyNumber);
      if (!claimStatus) {
        return;
      }

      const tx = await interactWithLifeContract(policyDetails, nomineeDetails, medicalDetails, bankDetails);
  ...
  const handleVehicleSubmit = async (e) => {
    ...
    const accidentDetails = {
      ...
      supportingDocument: e.target.supportingDocuments?.value || ""
    };
    ...
    try {
      const claimStatus = await interactWithGlobalClaimTracker(ClaimType.Vehicle, policyDetails.policyNumber);
      if (!claimStatus) {
        alert("This policy number has already been claimed.");
        return;
      }
      
      const tx = await interactWithVehicleContract(policyDetails, vehicleDetails, driverDetails, accidentDetails, bankDetails);
      const receipt = await tx.wait();
  ...
  const handleTravelSubmit = async (e) => {
    ...
    const policyDetails = {
      ...
      email: e.target.email?.value || "",
      policyStartDate: e.target.policyStartDate?.value || "",
      policyEndDate: e.target.policyEndDate?.value || "",
      insuranceCompany: e.target.insuranceCompany?.value || "",
      brokerName: e.target.agentBrokerName?.value || ""
    };
    ...
    const claimDetails = {
      typeOfClaim: e.target.typeOfClaim?.value || "",
      descriptionOfEvent: e.target.descriptionOfIncident?.value || "",
      amountClaimed: e.target.totalAmountClaimed?.value || "",
      attachedDocument: e.target.supportingDocuments?.value || ""
    };
    ...
    const declarationDetails = {
      ...
      declarationDate: e.target.declarationDate?.value || ""
    };
    ...
    try {
      ...
      const tx = await interactWithTravelContract(policyDetails, travelDetails, claimDetails, bankDetails, declarationDetails);
      const receipt = await tx.wait();
      ...
      await sendClaimToBackend("Travel", policyDetails.insuranceCompany, policyDetails.policyNumber);
      alert("Claim submitted successfully!");
    } catch (error) {
      console.error("Error filing claim:", error);
      alert("Error filing claim, please try again.");
